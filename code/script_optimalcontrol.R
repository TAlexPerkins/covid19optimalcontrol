...
# values of c to range across
c.vals = 10^(-12:-6)

# specify jobs that need to be run
jobs = expand.grid(pp = 1:nrow(calib), cc = 1:length(c.vals))
...
toSave = foreach(ii = 1:nrow(jobs), .packages='deSolve') %dopar% {
  ...
  # set value of parameter for weighting costs
  parameters['c'] = c.vals[cc]
  ...
  umin = c(umax[times<=121],rep(0,sum(times>121)))
...
# save results
save(toSave,file='../data/sweep_objectives_lite.RData')
stopCluster(cl)
...
# plot information about convergence
png('../figures/convergence.png',width=6.5,height=2.75,units='in',res=200)

  # retrieve iterative solutions and their objective values
  scenarios = cbind(
    R0 = calib[jobs[,'pp'],'R0'],
    umax = calib[jobs[,'pp'],'umax'],
    omega = calib[jobs[,'pp'],'omega'],
    c = c.vals[jobs[,'cc']])
  ii = which(
    scenarios[,'R0']== 3.0 &
      scenarios[,'umax']== 0.9 &
      ...
      scenarios[,'c']== 1e-12)
  J = toSave[[ii]]$J
  best.J = 1900+rev(sort(order(tail(J,100),decreasing=F)[1:10],decreasing=T))

  layout(matrix(1:2,1,2))
  par(mar=c(4.5,6,1.5,1),oma=rep(0,4))

  # plot control solutions across iterations
  image.plot(
    x = 1:nrow(toSave[[ii]]$umat),
    y = 1:ncol(toSave[[ii]]$umat),
    z = toSave[[ii]]$umat,
    xlab='Iteration',ylab='Time',las=1)

  # plot objective across iterations
  plot(J,type='l',log='y',xlab='Iteration',ylab='Objective, J(u)')
...
pdf('../figures/objective_sensitivity.pdf',width=6.5,height=5.5)
  ...
  for(ll in 1:3){
    for(kk in 1:3){
      ...
      plot(sapply(which.plot,function(ii)toSave[[ii]]$parameters['c']),
           sapply(which.plot,function(ii)toSave[[ii]]$u),log='x',type='b',pch=4,col=4,
           xlab='',ylab='',las=1,yaxt='n',ylim=c(0,500))
      axis(2,las=1,col=4,col.ticks=4,col.axis=4)
      which.plot = which(
        sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['R0']})==R0.vec[ll] &
          sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['umax']})==umax.vec[kk] &
          sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['omega']})==0.8)
      lines(sapply(which.plot,function(ii)toSave[[ii]]$parameters['c']),
            sapply(which.plot,function(ii)toSave[[ii]]$u),log='x',type='b',pch=3,lty=3,col=4)
      par(new=T)
      which.plot = which(
        sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['R0']})==R0.vec[ll] &
          sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['umax']})==umax.vec[kk] &
          sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['omega']})==0.5)
      plot(sapply(which.plot,function(ii)toSave[[ii]]$parameters['c']),
           sapply(which.plot,function(ii)toSave[[ii]]$D),log='x',type='b',pch=4,col=2,
           xaxt='n',yaxt='n',xlab='',ylab='',ylim=c(0,0.06))
      ...
      which.plot = which(
        sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['R0']})==R0.vec[ll] &
          ...
          sapply(1:length(toSave),function(jj){toSave[[jj]]$parameters['omega']})==0.8)
      lines(sapply(which.plot,function(ii)toSave[[ii]]$parameters['c']),
            sapply(which.plot,function(ii)toSave[[ii]]$D),log='x',type='b',pch=3,lty=3,col=2)
      eval(parse(text=paste(
        'mtext(expression(R[0]*" = ',R0.vec[ll],', "*u["max"]*" = ',umax.vec[kk],'"),3,cex=0.7,line=0.25)',sep='')))
      if(kk==3 & ll==2){
        mtext('Weighting of the control in the objective function, c',1,cex=0.7,line=2.75)
      }
      if(kk==2 & ll==1){
...
for(ii in 1:length(toSave)){
  ...
  label = paste(paste(c('R0','umax','omega'),round(10*toSave[[ii]]$parameters[c('R0','umax','omega')]),sep='_',collapse='_'),'c',toSave[[ii]]$parameters['c'],sep='_')
  outState = toSave[[ii]]$outState
  parameters = toSave[[ii]]$parameters
  u = approxfun(times,toSave[[ii]]$umat[best[ii,'iteration'],],rule=2)

  # plot outputs
  png(file=paste('../figures/output_',label,'.png',sep=''),width=3.25,height=3.25,units='in',res=200)
...
# plot the relationship between hospitalizations and probability of death
pdf('../figures/death_prob.pdf',width=3.25,height=3.25)
  par(mar=c(5.1,4.1,1.5,2.1))
  H = seq(0,0.005,length.out=500)
  plot(
    H,
    Dfun(H,t=0,eta=parameters['eta'],parameters['Delta.min'],parameters['Delta.max'],parameters['h'])/
      H/parameters['eta'],
    type='l',ylim=c(0,parameters['Delta.max']),ylab='Pr( Death | Hospitalization )',xaxs='i',las=1,
    xlab='Hospitalizations, H(t)')
  abline(v=parameters['Hmax'],lty=2)
  mtext(expression(H['max']),3,at=parameters['Hmax'])
  mtext(expression(Delta['-']),4,at=parameters['Delta.min'],las=1,line=0.5)
  mtext(expression(Delta['+']),4,at=parameters['Delta.max'],las=1,line=0.5)
dev.off()
